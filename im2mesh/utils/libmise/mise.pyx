...
cdef class MISE:
    ...
    def update_with_label(self, long[:, :] points, double[:] values, int[:] labels):
        """Update points and set their values. Also determine all active voxels and subdivide them."""
        assert(points.shape[0] == values.shape[0])
        ...
        for i in range(points.shape[0]):
            ...
            if idx == -1:
                raise ValueError('Point not in grid!')
            self.grid_points[idx].value = values[i]
            self.grid_points[idx].label = labels[i]
            self.grid_points[idx].known = True
        # Subdivide activate voxels and add new points
        self.subdivide_voxels()

    def update_with_label_trans(self, long[:, :] points, double[:] values, int[:] labels, double[:, :] coords):
        """Update points and set their values. Also determine all active voxels and subdivide them."""
        assert(points.shape[0] == values.shape[0])
        assert(points.shape[0] == labels.shape[0])
        assert(points.shape[0] == coords.shape[0])
        assert(points.shape[1] == 3)
        ...
        for i in range(points.shape[0]):
            ...
            self.grid_points[idx].value = values[i]
            self.grid_points[idx].label = labels[i]
            self.grid_points[idx].x = coords[i, 0]
            self.grid_points[idx].y = coords[i, 1]
            self.grid_points[idx].z = coords[i, 2]
            self.grid_points[idx].known = True
        # Subdivide activate voxels and add new points
        self.subdivide_voxels()
    ...
    def to_dense_with_label(self):
        """Output dense matrix at highest resolution."""
        out_array = np.full((self.resolution + 1,) * 3, np.nan)
        label_array = np.full((self.resolution + 1,) * 3, np.nan, dtype=np.int32)
        cdef double[:, :, :] out_view = out_array
        cdef int[:, :, :] label_view = label_array
        cdef GridPoint point
        cdef int i, j, k
        ...
        for point in self.grid_points:
            ...
            # assert(point.known)
            out_view[point.loc.x, point.loc.y, point.loc.z] = point.value
            label_view[point.loc.x, point.loc.y, point.loc.z] = point.label
        ...
        for i in range(1, self.resolution + 1):
            for j in range(self.resolution + 1):
                for k in range(self.resolution + 1):
                    if isnan(out_view[i, j, k]):
                        out_view[i, j, k] = out_view[i-1, j, k]
                        label_view[i, j, k] = label_view[i-1, j, k]
        ...
        for i in range(self.resolution + 1):
            for j in range(1, self.resolution + 1):
                for k in range(self.resolution + 1):
                    if isnan(out_view[i, j, k]):
                        out_view[i, j, k] = out_view[i, j-1, k]
                        label_view[i, j, k] = label_view[i, j-1, k]
        ...
        for i in range(self.resolution + 1):
            for j in range(self.resolution + 1):
                for k in range(1, self.resolution + 1):
                    if isnan(out_view[i, j, k]):
                        out_view[i, j, k] = out_view[i, j, k-1]
                        label_view[i, j, k] = label_view[i, j, k-1]

                    assert(not isnan(out_view[i, j, k]))
                    assert(not isnan(label_view[i, j, k]))

        return out_array, label_array

    def to_dense_with_label_trans(self):
        """Output dense matrix at highest resolution."""
        out_array = np.full((self.resolution + 1,) * 3, np.nan)
        label_array = np.full((self.resolution + 1,) * 3, np.nan, dtype=np.int32)
        trans_array = np.full([self.resolution + 1,] * 3 + [3], np.nan)
        cdef double[:, :, :] out_view = out_array
        cdef int[:, :, :] label_view = label_array
        cdef double[:, :, :, :] trans_view = trans_array
        cdef GridPoint point
        cdef int i, j, k
        ...
        for point in self.grid_points:
            ...
            out_view[point.loc.x, point.loc.y, point.loc.z] = point.value
            label_view[point.loc.x, point.loc.y, point.loc.z] = point.label
            trans_view[point.loc.x, point.loc.y, point.loc.z, 0] = point.x
            trans_view[point.loc.x, point.loc.y, point.loc.z, 1] = point.y
            trans_view[point.loc.x, point.loc.y, point.loc.z, 2] = point.z
            ...
            for j in range(self.resolution + 1):
                for k in range(self.resolution + 1):
                    if isnan(out_view[i, j, k]):
                        out_view[i, j, k] = out_view[i-1, j, k]
                        label_view[i, j, k] = label_view[i-1, j, k]
                        trans_view[i, j, k, 0] = trans_view[i-1, j, k, 0] 
                        trans_view[i, j, k, 1] = trans_view[i-1, j, k, 1] 
                        trans_view[i, j, k, 2] = trans_view[i-1, j, k, 2] 
            ...
            for j in range(1, self.resolution + 1):
                for k in range(self.resolution + 1):
                    if isnan(out_view[i, j, k]):
                        out_view[i, j, k] = out_view[i, j-1, k]
                        label_view[i, j, k] = label_view[i, j-1, k]
                        trans_view[i, j, k, 0] = trans_view[i, j-1, k, 0] 
                        trans_view[i, j, k, 1] = trans_view[i, j-1, k, 1] 
                        trans_view[i, j, k, 2] = trans_view[i, j-1, k, 2] 
            ...
            for j in range(self.resolution + 1):
                for k in range(1, self.resolution + 1):
                    if isnan(out_view[i, j, k]):
                        out_view[i, j, k] = out_view[i, j, k-1]
                        label_view[i, j, k] = label_view[i, j, k-1]
                        trans_view[i, j, k, 0] = trans_view[i, j, k-1, 0] 
                        trans_view[i, j, k, 1] = trans_view[i, j, k-1, 1] 
                        trans_view[i, j, k, 2] = trans_view[i, j, k-1, 2] 

                    assert(not isnan(out_view[i, j, k]))
                    assert(not isnan(label_view[i, j, k]))
                    assert(not isnan(trans_view[i, j, k, 0]))
                    assert(not isnan(trans_view[i, j, k, 1]))
                    assert(not isnan(trans_view[i, j, k, 2]))

        return out_array, label_array, trans_array
